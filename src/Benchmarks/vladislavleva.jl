function grammar_vladislavleva_b(; erc=false)
...
gt_vladislavleva_3(x::Float64, y::Float64) = exp(-x)*x^3*(cos(x)*sin(x))*(cos(x)*sin(x)^2-1)*(y-5)
...
gt_vladislavleva_6(x::Float64, y::Float64) = 6.0*sin(x)*cos(y) 
function loss_vladislavleva_6(S::SymbolTable, tree::RuleNode, grammar::Grammar)
    ex = get_executable(tree, grammar)
    los = 0.0
    n = 0
    for x in range(0.1,stop=5.9,length=30) 
        S[:x] = x
        for y in range(0.1,stop=5.9,length=30) 
            S[:y] = y
            los += abs2(Core.eval(S,ex)::Float64 - gt_vladislavleva_6(x,y))
            n += 1
        end
    end
    los / n
...
function run_vladislavleva_6(seed::Int=0, n_pop::Int=300)
    Random.seed!(seed)

    grammar = grammar_vladislavleva_b(erc=true)
    S = SymbolTable(grammar, ExprOptimization.Benchmarks)
    loss(tree::RuleNode, grammar::Grammar) = loss_vladislavleva_6(S, tree, grammar)
