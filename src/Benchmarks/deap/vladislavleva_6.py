...
def gt(x, y):
    return 6.0*protectedSin(x)*protectedCos(y) 
...
def evalSymbReg(individual):
    ...
    los = 0.0
    n = 0
    for x in numpy.linspace(0.1,5.9,30):
        for y in numpy.linspace(0.1,5.9,30):
...
def main():
    n_seeds = 50
    with open('../results/deap_vladislavleva_6.csv', 'wb') as csvfile:
        ...
        for n_pop in pop_sizes:
            ...
            w.writerow(['deap', 'vladislavleva_6', n_seeds, n_pop, mean_time_s, std_time_s, mean_fitness, std_fitness])
